...
import { GET_SPENT_BY_TAG_IN_DATE_RANGE } from "../queries";
...
const SpendByTag = () => {
  ...
  const { data: { get_spent_by_tag_in_date_range: rawData } = {} } = useQuery(
  ...
  useEffect(() => {
    // If rawData changes we should update the selected tags.
    if (rawData) {
